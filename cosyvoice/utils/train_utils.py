...
def _cleanup_old_ckpt(model_dir, model_name_pattern, keep=MAX_CKPT_KEEP):
    ...
    """
    ckpt_files = sorted(glob.glob(os.path.join(model_dir, model_name_pattern)),
...
def save_model(model, model_name, info_dict):
    ...
    if rank == 0:
        ...
        logging.info('[Rank 0] Checkpoint saved: %s  (auto clean-up, keep latest %d)',save_model_path, MAX_CKPT_KEEP)
